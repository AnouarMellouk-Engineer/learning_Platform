...
const Nav = () => {
  ...
  return (
    <nav className=" phone-container md:px-0 py-3.5  bg-background-secondery w-full fixed top-0  ">
      <div className="  md:container  mx-auto font-heading flex items-center justify-between">
        <h1 className="text-3xl font-medium cursor-pointer flex-1">Elearing</h1>
        <div
        ...
        >
          ...
          <ul className="  flex  flex-col  md:flex-row items-center justify-center gap-8">
            {["About", "Courses", "Tracks", "Contact"].map((link) => (
              <li
                key={link}
                className="text-md cursor-pointer duration-300 hover:text-primary"
              >
                {link}
              </li>
            ))}
          </ul>
          <button className="block px-8 py-3 bg-primary duration-300  rounded-full text-white text-lg font-text cursor-pointer">
            Register
          </button>
        </div>
